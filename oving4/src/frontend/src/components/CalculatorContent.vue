<template>
  ...
  <div class="container">
    <div class="calculator">
      ...
      <div @click="answer()" class="btn ans">ans</div>
      <div @click="equal" class="btn operator equal">=</div>
    </div>
    <div class="log">
      <h2>Log</h2>
      <div class="equation-wall" ref="equationWall">
        <div v-for="equation in equations" :key="equation" class="equation">
          {{ equation }}
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    equal() {
      axios
        .post("http://localhost:8080/api/calculator/solve", {
          expression: this.current,
        })
        .then((response) => {
          this.equations.push(this.current + " = " + response.data.result);
          this.current = "";
        })
        .catch((error) => {
          console.log(error);
          this.equations.push("Error in calculation");
          this.current = "Error";
        });
    },
    answer() {
